...
def createtable():
    ...
    try:
        cur.execute(
            "CREATE TABLE movies ( id int(50) NOT NULL auto_increment, name varchar(50), release_date date, director varchar(100), lead_actor varchar(50), lead_actress varchar(50), PRIMARY KEY(id) );")
    except Exception as e:
        conn.rollback()
        # print(e)
...
def find(query):
    ...
    try:
        cur.execute(
            "SELECT name, release_date, director, lead_actor, lead_actress FROM movies where lead_actor LIKE '%" + query + "%' or lead_actress LIKE '%" + query + "%' ORDER BY release_date ASC")
        records = cur.fetchall()
        if records:
            head = ["Name", "Release Date", "Director", "Actors", "Actress"]
...
def insert(name, release_date, director, actor, actress):
    ...
    cur = conn.cursor()
    try:
        cur.execute("INSERT INTO movies (name,release_date,director,lead_actor,lead_actress) VALUES (%s,%s,%s,%s,%s)",
                    (name, release_date, director, actor, actress))
        print("Data Inserted!!!")
    except Exception as e:
        print(e)
    finally:
